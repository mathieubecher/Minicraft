...
public:
	...
	void render_world_vbo(int shader, YTexFile * texture, bool debug, bool doTransparent, YVec3f sunpos, YVec3f avatar, float time)
	{
		...
		var = glGetUniformLocation(shader, "avatar");
		glUniform3f(var, avatar.X, avatar.Y, avatar.Z);

		int i = 0;
		bool endDel = false;
		while (ondes.size()>20*3 && !endDel) {
			ondes.pop_front();
			ondes.pop_front();
			ondes.pop_front();
		}

		GLfloat ondesTab[20*3];
		std::copy(ondes.begin(),ondes.end(),ondesTab);
		var = glGetUniformLocation(shader, "ondesSize");
		glUniform1f(var, ondes.size()/3);

		var = glGetUniformLocation(shader, "ondes");
		glUniform3fv(var,ondes.size(),ondesTab);
